...
if not os.path.exists("config.json"):
   ...
   # No config.json exist means that browser and tor are not pointed for the program yet
...
def send_file(name):
   ...
   # TODO: need to add file size limit (Tor network reset and zipbomb protection)
...
def receive_message():
   ...
   The server (/receive_message) doesn't trust the onion_address automatically,
   ...
   on the server, and then changes the OTP to another value. Otherwise, it returns a 400.
